...
<body class="bg-white text-black font-sans">
	...
	<main class="max-w-screen-xl mx-auto px-6 mt-10 mb-20">
		...
		<div class="flex justify-end mb-4">
			<a href="admin_rental_product.go" class="bg-black text-white px-4 py-2 rounded hover:bg-gray-800 transition">대여 상품 재고 현황</a>
		</div>
			...
			<table class="min-w-full text-sm text-center border border-gray-200">
				<thead class="bg-gray-100">
					<tr>
						...
						<th class="border px-4 py-2">회원명</th>
						<th class="border px-4 py-2">상품</th>
							...
							<c:forEach var="rental" items="${rentalList}">
								<c:if test="${rental.rental_status ne '반납완료'}">
									<tr class="${rental.remaining_days <= 2 ? 'bg-red-100 text-red-700 font-bold' : ''}">
										...
										<td class="border px-4 py-2 text-center">${rental.product_name}</td>
										...
										<td class="border px-4 py-2">
											<c:choose>
												<c:when test="${rental.remaining_days lt 0}">
													${rental.remaining_days}일
												</c:when>
												<c:otherwise>
													${rental.remaining_days}일
												</c:otherwise>
